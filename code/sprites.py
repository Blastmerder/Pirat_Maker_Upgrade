...
class Tooth(Generic):
    ...
    def move(self, dt):
        ...
        if self.direction.x > 0:
            floor_sprite = [sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(right_gap)]
            wall_sprite = [sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(right_block)]

            if not floor_sprite or wall_sprite:
                self.direction *= -1
                self.orientation = 'left'
        elif self.direction.x < 0:
            floor_sprite = [sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(left_gap)]
            wall_sprite = [sprite for sprite in self.collision_sprites if sprite.rect.collidepoint(left_block)]
...
class Player(Generic):
    def __init__(self, pos, assets, group, collision_spites, jump_sound):
        ...
        # collision
        self.collision_sprites = collision_spites
    ...
    def get_status(self):
        if self.direction.y < 0:
            self.status = 'jump'
        elif self.direction.y > 1 and not self.on_floor:
            self.status = 'fall'
        else:
    ...
    def move(self, dt):
        ...
        # vertical
        self.pos.y += self.direction.y * self.speed * dt if self.direction.y > 0 and not self.on_floor else 0
        self.pos.y += self.direction.y * self.speed * dt if self.direction.y < 0 else 0
        self.hitbox.centery = round(self.pos.y)
        self.rect.centery = self.hitbox.centery
        self.collision('vertical')

    def apply_gravity(self, dt):
        if self.direction.y > 0 and self.on_floor:
            self.direction.y = 0
