...
class Editor:
    ...
    def imports(self):
        ...
        self.water_bottom = pygame.transform.rotozoom(self.water_bottom, 0, TILE_SIZE / 64)
    ...
    def menu_click(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and self.menu.rect.collidepoint(mouse_pos()):
            new_index = self.menu.click(mouse_pos(), mouse_button())
    ...
    def object_drag(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and mouse_button()[0]:
            for sprite in self.canvas_objects:
    ...
    def preview(self):
        ...
        if not self.menu.rect.collidepoint(mouse_pos()):
            if selected_object:
                ...
                # topright
                pygame.draw.lines(self.display_surface, color, False,
                                  ((rect.right, rect.top + size), rect.topright, (rect.right - size, rect.top)), width)
                # bottom right
                pygame.draw.lines(self.display_surface, color, False, (
